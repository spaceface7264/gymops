...
| P7M-08 | Pull-to-refresh on shell pages on a phone: `usePullToRefresh` + `PullIndicator`, refetches every active query, Chrome's native reload off below `md` | P7M-07     | S      |
...
- (Acknowledgement reminders were listed here as missing; `send_ack_reminders()` has raised them nightly since P5-02, confirmed by the P7-07 walkthrough on 2026-09-04. Branded auth mail templates were listed too and landed as P9-02.)
- Report an incident (and write a log entry) from the "All gyms" scope: show the button anyway and, with no gym selected, open a gym picker first instead of hiding the action. Found on the hosted walkthrough 2026-09-06 — the superadmin looked for the button and could not see why it was gone.
