...
| `EmptyState`    | nothing here yet: `title`, optional `icon`, `body`, `action`                                                                                                               | a sentence, not a box: no border, and the icon only from `md` up. On a screen with no other heading pass `as="h1"`. Reuse the existing `*.empty` key as the title.                 |
| `LoadError`     | a list or screen that did not load: `message`, `onRetry`                                                                                                                   | the only way a load failure is shown: one line and a 44 px Try again calling `refetch()`; never a bare `role="alert"` paragraph for a query                                        |
...
| `HomeSection`   | one block of the home page: `title`, optional `action` (a `HomeSectionLink`), rows as `HomeRows` of `HomeRow` (a 44 px link with `badge`, `meta`), or one `HomeEmpty` line | a heading with rows, not a card; an empty block is one muted line. The manager's week card is the one card on the page                                                             |
| `Sheet`         | (in `ui/`) a panel from an edge; the phone bar's More opens one from the bottom                                                                                            | Radix Dialog like `dialog`: card surface, 150 ms, `SheetTitle` names it; rows inside are 44 px                                                                                     |
