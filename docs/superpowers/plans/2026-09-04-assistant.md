...
- **Cap = calls per user per UTC day**, one `assistant_usage` row per call (reserved *before* the model call, tokens updated after), cap value in a new `app_settings` table (`assistant_daily_cap`, default 50).
