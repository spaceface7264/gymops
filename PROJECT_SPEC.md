...
gymops/
  src/
    ...
    hooks/                  app-wide hooks (a media query for DOM that differs by width, pull-to-refresh)
...
| A 16 px root to get 44 px controls                                        | The 15 px body was a decided step for a phone at arm's length; a 16 px root would move every type size with it. `--spacing: 4px` fixes the one thing that was wrong (P7M-01). |
...
| A pull-to-refresh library, or relying on Chrome's native pull-to-reload | Decided 2026-09-06 (P7M-08): the standalone PWA has no reload button and queries are stale for 30 s with no refetch on focus, so the phone needs the gesture. A library brings a dependency and its own styling; Chrome's native one reloads the whole page, drops SPA state, and iOS standalone has nothing. `usePullToRefresh` is about 90 lines on passive document listeners, refetches every active query, and `overscroll-behavior-y: none` below `md` keeps Chrome's own from firing beside it. Shell pages only: chat is full-bleed, scrolls inside itself and is live. The More sheet carries a Refresh row as the single-pointer alternative (WCAG 2.5.1), and `useRefetchOnResume()` refetches stale queries on `visibilitychange` so the gesture is the fallback, not the data path; `refetchOnWindowFocus` stays off because some browsers fire focus on every click into the window. |
