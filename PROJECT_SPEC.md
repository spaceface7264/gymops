...
- **Team chat:** auto channel per gym + `#company`, custom public/private channels, DMs (2+ people), @mentions, image/file attachments, delete own messages (edit was removed 2026-09-05, §4), unread badges, typing presence. No threads, reactions or search in V1.
...
| Bubbles for every chat channel | Rejected 2026-09-05 (P6C-12): a gym or company channel has 10 to 30 speakers and long handovers, where alternating bubbles at 80 % width read worse than rows. Bubbles are for DMs only, where the side says who is talking; own lines in the `accent` tint, the other person's on white. |
| A `…` menu per chat message holding one item | Replaced 2026-09-05 (P6C-09, second critique): a menu that opens to a single red Delete is indirection with no payoff. Delete is a direct icon button, hover-revealed from `md` up and always shown on a phone; the menu returns if a second action (copy, reply) ever joins it. |
